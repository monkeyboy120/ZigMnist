...
pub fn avgPool(self: PoolLayer, input: Tensor, allocator: *std.mem.Allocator) !Tensor {
    const in_h = input.shape[0];
    const in_w = input.shape[1];
    const in_c = input.shape[2];
    ...
    for (out_h) |oh| {
        for (out_w) |ow| {
            for (in_c) |ic| {
                ...
                // Set the average output tensor
                try output.set(oh, ow, ic, sum / f64(self.pool_size * self.pool_size));
            }
        }
    }
    return output;
